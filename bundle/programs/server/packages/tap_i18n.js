...
_.extend(TAPi18n.prototype, {                                                                                         // 1
  ...
  _registerServerTranslator: function (lang_tag, package_name) {                                                      // 4
    if (this._enabled()) {                                                                                            // 5
  ...
  _registerAllServerTranslators: function () {                                                                        // 16
    var i, lang_tag, len, package_name, ref, results;                                                                 // 17
    ref = this._getProjectLanguages();                                                                                // 17
    results = [];                                                                                                     // 17
                                                                                                                      //
    for (i = 0, len = ref.length; i < len; i++) {                                                                     // 22
      lang_tag = ref[i];                                                                                              // 23
      results.push(function () {                                                                                      // 24
        var results1;                                                                                                 // 25
        results1 = [];                                                                                                // 18
                                                                                                                      //
        for (package_name in meteorBabelHelpers.sanitizeForInObject(this.translations[lang_tag])) {                   // 27
          results1.push(this._registerServerTranslator(lang_tag, package_name));                                      // 28
        }                                                                                                             // 18
                                                                                                                      //
        return results1;                                                                                              // 30
      }.call(this));                                                                                                  // 31
  ...
  _getPackageI18nextProxy: function (package_name) {                                                                  // 21
    return function (_this) {                                                                                         // 36
      return function (key, options, lang_tag) {                                                                      // 37
        if (lang_tag == null) {                                                                                       // 38
          lang_tag = null;                                                                                            // 23
        }                                                                                                             // 40
                                                                                                                      //
        ...
        } else if (!(lang_tag in _this.server_translators)) {                                                         // 24
  ...
  _registerHTTPMethod: function () {                                                                                  // 33
    var methods, self;                                                                                                // 34
    self = this;                                                                                                      // 34
    methods = {};                                                                                                     // 36
                                                                                                                      //
    if (!self._enabled()) {                                                                                           // 38
      throw new Meteor.Error(500, "tap-i18n has to be enabled in order to register the HTTP method");                 // 39
                                                                                                                      ...
                                                                                                                      //
    methods[self.conf.i18n_files_route.replace(/\/$/, "") + "/multi/:langs"] = {                                      // 41
      get: function () {                                                                                              // 42
        ...
        if (!RegExp("^((" + globals.langauges_tags_regex + ",)*" + globals.langauges_tags_regex + "|all).json$").test(this.params.langs)) {
          return this.setStatusCode(401);                                                                             // 44
                                                                                                                      ...
                                                                                                                      //
        langs = this.params.langs.replace(".json", "");                                                               // 46
                                                                                                                      //
        if (langs === "all") {                                                                                        // 48
          output = self.translations;                                                                                 // 49
        } else {                                                                                                      // 48
          output = {};                                                                                                // 51
          langs = langs.split(",");                                                                                   // 53
                                                                                                                      //
    ...
    methods[self.conf.i18n_files_route.replace(/\/$/, "") + "/:lang"] = {                                             // 64
      get: function () {                                                                                              // 65
